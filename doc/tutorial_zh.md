...
### 下载[release binary](https://github.com/feixiao/codis/releases)文件安装
...
Codis 源代码需要下载到 `$GOPATH/src/github.com/feixiao/codis`：
...
对 Java 用户来说，可以使用基于 Jedis 的实现 [Jodis](https://github.com/feixiao/jodis) ，来实现 proxy 层的 HA：
    
+ 它会通过监控 zookeeper 上的注册信息来实时获得当前可用的 proxy 列表，既可以保证高可用性；
+ 也可以通过轮流请求所有的proxy实现负载均衡。

如果需要异步请求，可以使用我们基于Netty开发的 [Nedis](https://github.com/feixiao/nedis)。
